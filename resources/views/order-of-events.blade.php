<!doctype html>
<html>
@section('title', 'Order Of Event')
@include('partials.head')
<body class="bg-rose-light font-sans">

<div class="max-w-md mx-auto bg-rose-light min-h-screen px-4">

    @include('partials.header')

    <h1 class="text-maroon text-4xl text-center mt-24 mb-8">Order of Event</h1>

    <section class="mb-6 rounded-lg overflow-hidden">
        <div class="bg-rose-medium p-2 text-center">
            <div class="flex justify-between items-center mb">
                <img src="{{asset('/img/face-left.svg')}}" alt="" class="w-12 h-12">
                <div class="flex flex-col items-center">
                    <h2 class="text-maroon text-2xl font-bold">Traditional Marriage</h2>
                    <p class="text-maroon font-sans">10:00am-11:30am</p>

                </div>

                <img src="{{asset('/img/face-right.svg')}}" alt="" class="w-12 h-12">
            </div>

        </div>
        <div class="bg-maroon p-6">
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>By Compere</span>
            </div>
        </div>
    </section>

    <section class="mb-6 rounded-lg overflow-hidden">
        <div class="bg-rose-medium p-2 text-center">
            <div class="flex justify-between items-center mb-2">
                <img src="{{asset('/img/face-left.svg')}}" alt="" class="w-12 h-12">
                <div>
                    <h2 class="text-maroon text-2xl font-bold">Nikkah</h2>
                    <p class="text-maroon">12:00pm-1:30pm</p>

                </div>

                <img src="{{asset('/img/face-right.svg')}}" alt="" class="w-12 h-12">
            </div>

        </div>
        <div class="bg-maroon p-6 space-y-3">
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Opening Dua</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Introduction of the officiating Imams</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Introduction of the Couple parents</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Arrival of the Groom</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Arrival of the Bride</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Reading of the Holy Quran</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Khuthbah of the importance of Marriage</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Exchange of Ring</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Sign marriage certificate</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Advice to the couple/Prayer</span>
            </div>
        </div>
    </section>


    <section class="mb-6 rounded-lg overflow-hidden">
        <div class="bg-rose-medium p-2 text-center">
            <div class="flex justify-between items-center mb-2">
                <img src="{{asset('/img/face-left.svg')}}" alt="" class="w-12 h-12">
                <div>
                    <h2 class="text-maroon text-2xl font-bold">Reception</h2>
                    <p class="text-maroon font-sans">2:00pm-6:00pm</p>
                </div>
                <img src="{{asset('/img/face-right.svg')}}" alt="" class="w-12 h-12">
            </div>
        </div>
        <div class="bg-maroon p-6 space-y-3">
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Opening Introduction- Father/Mother of the day</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Couple Entrance</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Couple First Dance</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Father and Daughter Dance</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Games</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Dance Competition - Bride maid and Grooms men</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Gift Presentation</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Vote of thanks</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Groom family speach</span>
            </div>
            <div class="flex items-center gap-3 text-white">
                <img src="{{asset('/img/ring.svg')}}" alt="" class="w-5 h-5">
                <span>Dance! Dance! Dance!</span>
            </div>
        </div>
    </section>

</div>
@include('partials.footer')
</body>
</html>
